...
Route::Route(int start, const std::vector<Graph::TransportationLine>& lines)
    : _start(start), _lines(lines)
{
    _stops.push_back(_start);
    for (const auto& edge : _lines) {
        _stops.push_back(edge.to);
    }
}
...
const std::vector<int>& Route::getStops() const {
    return _stops;
}
